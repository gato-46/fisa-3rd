...
public class Dept01DAO {
	//모든 부서 정보 검색
	/* PreparedStatement API특징
	 * - db에 요청이 되면 동작 방식
	 * 		문장 받고 -> 문법 검증 -> 실행
	 * 		매순간 요청시 마다 : 검증 작업 수행 - Statement
	 * 		최초 받고 매 요청시 재사용 - PreparedStatement(문법 검증 생략)
	 * 	
	 * - JPA & Mybatis라는 db연동 framework들의 기본 API 로 내부적으로 사용
	 */
	@Test
	...
	public ArrayList<Dept01DTO> getDept01All() throws SQLException{
		...
		try {
			...
			datas = new ArrayList<>(); 
			while(rs.next()) { //반복 횟수가 불 명확할때 주로 사용되는 반복문
				datas.add( new Dept01DTO(rs.getInt(1), rs.getString("dname"),
							  			 rs.getString(3)) );
			}
