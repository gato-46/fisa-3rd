...
<body>
	...
	step02. java 데이터를 화면에 출력
	<br>
	<%
	//servlet에서 데이터 저장 후 이동된 jsp 관점에서 학습이라 가정
	//test 데이터 구성 임시로 진행함
	request.setAttribute("rd1", "요청 데이터1");
	session.setAttribute("sd1", "세션 데이터1");
	...
	//dto들을 HashMap에 저장해서 세션에 또 저장
	%>

	1. ${requestScope.rd1}-${sessionScope.sd1}
	<br> 2. 배열 : ${requestScope.rd2[1]}
	<br>

	<%-- out.println(((People)session.getAttibute("sd2")).getName()) --%>
	3. DTO : ${sessionScope.sd2.name}
	<br>
...
%>
5. DTO > HashMap : ${requestScope.rd4.p1.name}<br>
...
6. DTO > ArrayList > HashMap : ${requestScope.rd5.al[2].name}<br>
	...
	<br><hr><br> 
	step01. 브라우저에 출력 tag : 연산 및 비교가 가능
